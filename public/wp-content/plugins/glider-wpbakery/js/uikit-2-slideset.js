...
(function(addon) {
    ...
    if (window.UIkit2mdp) {
        component = addon(UIkit2mdp);
    }

    if (typeof define == 'function' && define.amd) {
        define('uikit-slideset', ['uikit'], function(){
            return component || addon(UIkit2mdp);
        });
    }
    ...
    UI.component('slideset', {
        ...
        boot: function() {
            ...
            UI.ready(function(context) {
                ...
                UI.$('[data-mdp-slideset]', context).each(function(){

                    var ele = UI.$(this);

                    if(!ele.data('slideset')) {
                        UI.slideset(ele, UI.Utils.options(ele.attr('data-mdp-slideset')));
                    }
                });
            });
        },
        ...
        init: function() {
            ...
            this.list      = this.element.find('.mdp-slideset');
            this.nav       = this.element.find('.mdp-slideset-nav');
            ...
            $this.list.addClass('mdp-grid-width-1-'+$this.options.default);
            ...
            ['xlarge', 'large', 'medium', 'small'].forEach(function(bp) {
                ...
                $this.list.addClass('mdp-grid-width-'+bp+'-1-'+$this.options[bp]);
            });

            this.on('click.uk.slideset', '[data-mdp-slideset-item]', function(e) {
                ...
                var set = UI.$(this).attr('data-mdp-slideset-item');
            ...
            this.controls.on('click.uk.slideset', '[data-mdp-filter]', function(e) {

                var ele = UI.$(this);

                if (ele.parent().hasClass('mdp-slideset')) {
                    return;
                }

                e.preventDefault();

                if ($this.animating || $this.currentFilter == ele.attr('data-mdp-filter')) {
                    return;
                }

                $this.updateFilter(ele.attr('data-mdp-filter'));
            ...
            UI.domObserve(this.list, function(e) {
                if ($this.list.children(':visible:not(.mdp-active)').length) {
        ...
        update: function(animate, force) {
            ...
            if (this.nav.length && this.nav.empty()) {
                ...
                for (i=0;i<this.sets.length;i++) {
                    this.nav.append('<li data-mdp-slideset-item="'+i+'"><a></a></li>');
                }

                this.nav[this.nav.children().length==1 ? 'addClass':'removeClass']('mdp-invisible');
            }
        ...
        updateFilter: function(currentfilter) {
            ...
            this.controls.find('[data-mdp-filter]').each(function(){

                filter = UI.$(this);

                if (!filter.parent().hasClass('mdp-slideset')) {

                    if (filter.attr('data-mdp-filter') == $this.currentFilter) {
                        filter.addClass('mdp-active');
                    } else {
                        filter.removeClass('mdp-active');
                    }
                }
            });
        },
        ...
        getVisibleOnCurrenBreakpoint: function() {
            ...
            var breakpoint  = null,
                ...
                ['xlarge', 'large', 'medium', 'small'].forEach(function(bp) {
                    ...
                    tmp.attr('class', 'mdp-grid-width-'+bp+'-1-2').width();
        ...
        getItems: function() {
            ...
            if (this.currentFilter) {
                ...
                this.children.each(function(index){
                    ...
                    var ele = UI.$(this), f = ele.attr('data-mdp-filter'), infilter = filter.length ? false : true;
        ...
        show: function(setIndex, noanimate, dir) {
            ...
            if (this.nav.length) {
                this.nav.children().removeClass('mdp-active').eq(setIndex).addClass('mdp-active');
            }
            ...
            animation.apply($this, [current, next, dir]).then(function(){
                ...
                $this.children.hide().removeClass('mdp-active');
                next.addClass('mdp-active').css({'display': '', 'opacity':''});
    ...
    function coreAnimation(cls, current, next, dir) {
        ...
        release = function() {
            ...
            if (current && current.length) {
                current.hide().removeClass(clsOut+' mdp-animation-reverse').css({'opacity':'', 'animation-delay': '', 'animation':''});
            }
            ...
            for (i=0;i<current.length;i++) {
                ...
                (function (index, ele){
                    ...
                    setTimeout(function(){
                        ...
                        }).addClass(clsOut+' mdp-animation-reverse');
