...
final class WordPressReporter {
	...
	public function get_fields() {
		return [
			'version'               => esc_html__( 'Version', 'worker-wpbakery' ),
			'site_url'              => esc_html__( 'Site URL', 'worker-wpbakery' ),
			'home_url'              => esc_html__( 'Home URL', 'worker-wpbakery' ),
			'is_multisite'          => esc_html__( 'WP Multisite', 'worker-wpbakery' ),
			'max_upload_size'       => esc_html__( 'Max Upload Size', 'worker-wpbakery' ),
			'memory_limit'          => esc_html__( 'Memory limit', 'worker-wpbakery' ),
			'permalink_structure'   => esc_html__( 'Permalink Structure', 'worker-wpbakery' ),
			'language'              => esc_html__( 'Language', 'worker-wpbakery' ),
			'timezone'              => esc_html__( 'Timezone', 'worker-wpbakery' ),
			'admin_email'           => esc_html__( 'Admin Email', 'worker-wpbakery' ),
			'debug_mode'            => esc_html__( 'Debug Mode', 'worker-wpbakery' ),
		];
	}
	...
	public function get_memory_limit() {
		...
		if ( $memory_limit_bytes < $min_recommended_bytes ) {
			$result['recommendation'] = esc_html__( 'We recommend setting memory to at least 64M. For more information, ask your hosting provider.','worker-wpbakery' );
	...
	public function get_is_multisite() {
		return [
			'value' => is_multisite() ? esc_html__( 'Yes', 'worker-wpbakery' ) : esc_html__( 'No', 'worker-wpbakery' )
		];
	}
	...
	public function get_permalink_structure() {
		...
		if ( ! $structure ) {
			$structure = esc_html__( 'Plain', 'worker-wpbakery' );
		}
	...
	public function get_debug_mode() {
		return [
			'value' => WP_DEBUG ? esc_html__('Active', 'worker-wpbakery' ) : esc_html__('Inactive', 'worker-wpbakery' )
		];
	}
