...
final class PluginActivation {
	...
	public function display_status() {
		...
		$activation_tab = admin_url( 'admin.php?page=mdp_worker_wpbakery_settings&tab=activation' );
		?>

        <hr class="mdc-list-divider">
        <h6 class="mdc-list-group__subheader"><?php esc_html_e( 'CodeCanyon License', 'worker-wpbakery' ); ?></h6>
		...
		<?php if ( $this->is_activated() ) : ?>
            <a class="mdc-list-item mdp-activation-status activated" href="<?php echo esc_url( $activation_tab ); ?>">
                ...
                <span class="mdc-list-item__text"><?php esc_html_e( 'Activated', 'worker-wpbakery' ); ?></span>
            </a>
		<?php else : ?>
            <a class=" mdc-list-item mdp-activation-status not-activated" href="<?php echo esc_url( $activation_tab ); ?>">
                ...
                <span class="mdc-list-item__text"><?php esc_html_e( 'Not Activated', 'worker-wpbakery' ); ?></span>
            </a>
		<?php endif;
	...
	public function remote_validation( $purchase_code ) {
		...
		$url .= 'plugin=worker-wpbakery&'; // Plugin Name.
		$url .= 'domain=' . parse_url( site_url(), PHP_URL_HOST ) . '&'; // Domain Name.
		$url .= 'version=1.0.1&'; // Plugin version.
	...
	public function add_settings() {
		...
		/** Activation Tab. */
		register_setting( 'WorkerWPBakeryActivationOptionsGroup', 'envato_purchase_code_' . EnvatoItem::get_instance()->get_id() );
		add_settings_section( 'mdp_worker_wpbakery_settings_page_activation_section', '', null, 'WorkerWPBakeryActivationOptionsGroup' );
	...
	public function render_pid() {
        ...
        <div class="mdp-activation">
            ...
            <div class="mdp-activation-form">
                ...
                <h3><?php esc_html_e( 'Plugin Activation', 'worker-wpbakery' ); ?></h3>

				<?php
				/** Render input. */
				UI::get_instance()->render_input(
					$purchase_code,
					esc_html__( 'CodeCanyon purchase code', 'worker-wpbakery'),
					esc_html__( 'Enter your CodeCanyon purchase code. Allowed only one Purchase Code per website.', 'worker-wpbakery' ),
					[
	...
	public function render_FAQ() {
		?>
        <div class="mdc-accordion">

            <h3><?php esc_html_e( 'Activation FAQ\'S', 'worker-wpbakery' ); ?></h3>

            <div class="mdc-accordion-title">
                <i class="material-icons">help</i>
                <span class="mdc-list-item__text"><?php esc_html_e( 'Where is my Purchase Code?', 'worker-wpbakery' ); ?></span>
            </div>
            <div class="mdc-accordion-content">
                <p><?php esc_html_e( 'The purchase code is a unique combination of characters that confirms that you bought the plugin. You can find your purchase code in ', 'worker-wpbakery' ); ?>
                    <a href="https://1.envato.market/cc-downloads" target="_blank"><?php esc_html_e( 'your account', 'worker-wpbakery' );?></a>
					<?php esc_html_e( ' on the CodeCanyon. Learn more about ', 'worker-wpbakery' ); ?>
                    <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e( 'How to find your purchase code', 'worker-wpbakery' );?></a>
					<?php esc_html_e( ' .', 'worker-wpbakery');?>
                </p>
            </div>

            <div class="mdc-accordion-title">
                <i class="material-icons">help</i>
                <span class="mdc-list-item__text"><?php esc_html_e( 'Can I use one Purchase Code on multiple sites?', 'worker-wpbakery' ); ?></span>
            </div>
            <div class="mdc-accordion-content">
                <p>
					<?php esc_html_e( 'No, this is prohibited by license terms. You can use the purchase code on only one website at a time. Learn more about ', 'worker-wpbakery' ); ?>
                    <a href="https://1.envato.market/KYbje" target="_blank"><?php esc_html_e( 'Envato License', 'worker-wpbakery' );?></a>
					<?php esc_html_e( ' terms. ', 'worker-wpbakery' ); ?>
                </p>
            </div>

            <div class="mdc-accordion-title">
                <i class="material-icons">help</i>
                <span class="mdc-list-item__text"><?php esc_html_e( 'What are the benefits of plugin activation?', 'worker-wpbakery' ); ?></span>
            </div>
            <div class="mdc-accordion-content">
                <p>
					<?php esc_html_e( 'Activation of the plugin allows you to use all the functionality of the plugin on your site. In addition, in some cases, activating the plugin allows you to access additional features and capabilities of the plugin. Also, using an authored version of the plugin, you can be sure that you will not violate the license.', 'worker-wpbakery' ); ?>
                </p>
            </div>

            <div class="mdc-accordion-title">
                <i class="material-icons">help</i>
                <span class="mdc-list-item__text"><?php esc_html_e( 'What should I do if my Purchase Code does not work?', 'worker-wpbakery' ); ?></span>
            </div>
            <div class="mdc-accordion-content">
                <p>
					<?php esc_html_e( 'There are several reasons why the purchase code may not work on your site. Learn more why your ', 'worker-wpbakery' ); ?>
                    <a href="https://help.market.envato.com/hc/en-us/articles/204451834-My-Purchase-Code-is-Not-Working" target="_blank"><?php esc_html_e( 'Purchase Code is Not Working', 'worker-wpbakery' );?></a>
					<?php esc_html_e( ' .', 'worker-wpbakery');?>
                </p>
            </div>
