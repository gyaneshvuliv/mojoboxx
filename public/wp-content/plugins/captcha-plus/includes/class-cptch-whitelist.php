	...
	class Cptch_Whitelist extends WP_List_Table {
		...
		*/
		function __construct( $plugin_basename, $limit_attempts_info ) {
			...
			*/
			$this->date_filter_options = array(
				'all'	=> __( 'All dates', 'captcha-plus' ),
				'day'	=> __( 'Last 24 hours', 'captcha-plus' ),
				'week'	=> __( 'Last week', 'captcha-plus' ),
				'month'	=> __( 'Last month', 'captcha-plus' ),
				'year'	=> __( 'Last year', 'captcha-plus' )
			);
		...
		function display_content() { ?>
			<h1>
                <?php _e( 'Captcha Plus Whitelist', 'captcha-plus' ); ?>
                <?php if ( ! $this->disable_list ) { ?>
                    <form method="post" action="admin.php?page=captcha-whitelist.php" class="cptch_whitelist_add_new" style="display: inline;">
                        <button class="page-title-action" name="cptch_show_whitelist_form" value="on"<?php echo ( isset( $_POST['cptch_add_to_whitelist'] ) ) ? ' style="display: none;"' : ''; ?>><?php _e( 'Add New', 'captcha-plus' ); ?></button>
                    </form>
                <?php } ?>
            </h1>
			...
			if ( ! ( isset( $_REQUEST['cptch_show_whitelist_form'] ) || isset( $_REQUEST['cptch_add_to_whitelist'] ) ) ) { ?>
				<form method="post" action="admin.php?page=captcha-whitelist.php">
					<?php if ( $this->disable_list ) { ?>
						<p><?php echo __( 'Limit Attempts whitelist is activated.', 'captcha-plus' ) .
							' <a href="admin.php?page=limit-attempts-black-and-whitelist.php">' . __( 'View the whitelist', 'captcha-plus' ) . '</a> ' .
							__( 'or', 'captcha-plus' ) .
							' <a href="admin.php?page=captcha-plus.php">' . __( 'Enable default captcha whitelist', 'captcha-plus' ) . '</a>'; ?></p>
					<?php } else { ?>
						<div>
							<input type="submit" name="cptch_load_limit_attempts_whitelist" class="button" value="<?php _e( 'Load IP Address(-es)', 'captcha-plus' ); ?>" style="float: left;" <?php echo $disabled; ?> />
							<noscript>
								<div class="alignleft" style="margin: 0 10px;">
									<input type="submit" name="cptch_save_add_ip_form_button" class="button-primary" value="<?php _e( 'Save changes', 'captcha-plus' ); ?>" />
								</div>
							</noscript>
							<div class="clear"></div>
						</div>
						<div class="bws_info"><?php _e( 'Load IP addresses from the "Limit Attempts" plugin.', 'captcha-plus' ); ?> <?php echo ! empty( $disabled ) ? $limit_attempts_info['link'] : ''; ?></div>
			...
			<form class="form-table cptch_whitelist_form" method="post" action="admin.php?page=captcha-whitelist.php" style="margin: 10px 0;<?php echo ! ( isset( $_REQUEST['cptch_show_whitelist_form'] ) || isset( $_REQUEST['cptch_add_to_whitelist'] ) ) ? 'display: none;': ''; ?>">
				<div>
					<label><?php _e( 'IP to whitelist', 'captcha-plus' ); ?></label>
					<br />
					...
					<?php } ?>
					<div>
						<span class="bws_info" style="line-height: 2;"><?php _e( "Allowed formats", 'captcha-plus' ); ?>:&nbsp;<code>192.168.0.1</code></span>
						<br/>
						<span class="bws_info" style="line-height: 2;"><?php _e( "Allowed diapason", 'captcha-plus' ); ?>:&nbsp;<code>0.0.0.0 - 255.255.255.255</code></span>
					</div>
					<p>
						<input type="submit" id="cptch_add_to_whitelist_button" class="button-secondary" value="<?php _e( 'Add IP to whitelist', 'captcha-plus' ) ?>" />
						<?php wp_nonce_field( $this->basename, 'cptch_nonce_name' ); ?>
					</p>
				</div>
			</form>
			<?php if ( ! $this->disable_list) { ?>
				<form id="cptch_whitelist_search" method="post" action="admin.php?page=captcha-whitelist.php">
					<?php $this->search_box( __( 'Search item', 'captcha-plus' ), 'search_whitelisted_ip' );
		...
		*/
		function no_items() {
			$label = isset( $_REQUEST['s'] ) ? __( 'Nothing found', 'captcha-plus' ) : __( 'No IP in the whitelist', 'captcha-plus' ); ?>
			<p><?php echo $label; ?></p>
		<?php }
		...
		function column_ip( $item ) {
			...
			$actions	= array(
				'delete' => '<a href="' . wp_nonce_url( $url, "cptch_nonce_remove_{$item['id']}" ) . '">' . __( 'Delete', 'captcha-plus' ) . '</a>'
			);
		...
		function get_bulk_actions() {
			return $this->disable_list ? array() : array( 'cptch_remove'=> __( 'Delete', 'captcha-plus' ) );
		}
		/**
		...
		function extra_tablenav( $which ) { ?>
			...
			<input type="submit" class="button action" value="<?php _e( 'Filter', 'captcha-plus' ); ?>" />
		<?php }
				...
				if ( $valid_ip ) {
					...
					if ( is_null( $id ) ) {
						...
						if ( ! $wpdb->last_error ) {
							...
							$error = __( 'Some errors occurred.', 'captcha-plus' );
						}
					} else {
		...
		function get_limit_attempts_info() {
			...
			if ( 'active' == $this->la_info['status'] ) {
				$data = array(
					...
					'label'				=> __( 'use', 'captcha-plus' ) . '&nbsp;<a href="?page=' . $this->la_info['plugin_info']["TextDomain"] . '.php">' . __( 'the whitelist of', 'captcha-plus' ) . '&nbsp;' . $this->la_info['plugin_info']["Name"] . '</a>',
					'notice'			=> '',
					'disabled'			=> false,
				);
				...
				$data = array(
					'active'			=> false,
					'name'				=> $this->la_info['plugin_info']["Name"],
					'label'				=> sprintf( __( 'use the whitelist of %s', 'captcha-plus' ), $this->la_info['plugin_info']["Name"] ),
					...
					'disabled'			=> true,
					'link'				=> '<a href="plugins.php">' . __( 'Activate', 'captcha-plus' ) . '</a>'
				);
			} elseif ( 'not_installed' == $this->la_info['status'] ) {
				$data = array(
					'active'			=> false,
					'name'				=> 'Limit Attempts by BestWebSoft',
					'label'				=> sprintf( __( 'use the whitelist of %s', 'captcha-plus' ), 'Limit Attempts by BestWebSoft' ),
					'notice'			=> sprintf( __( 'you should install %s to use this functionality', 'captcha-plus' ), '<a href="https://bestwebsoft.com/products/wordpress/plugins/limit-attempts?k=9ab9d358ad3a23b8a99a8328595ede2e&pn=72&v=' . $cptch_plugin_info["Version"] . '&wp_v=' . $wp_version . '" target="_blank">Limit Attempts by BestWebSoft</a>' ),
					'disabled'			=> true,
					'link'				=> '<a href="https://bestwebsoft.com/products/wordpress/plugins/limit-attempts?k=9ab9d358ad3a23b8a99a8328595ede2e&pn=72&v=' . $cptch_plugin_info["Version"] . '&wp_v=' . $wp_version . '" target="_blank">' . __( 'Install Now', 'captcha-plus' ) . '</a>'
				);
			}
			return $data;
		}
