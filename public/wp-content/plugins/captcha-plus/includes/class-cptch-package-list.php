 ...
 * @package Captcha Plus Pro by BestWebSoft
 * @since 1.6.9
 */
	...
	class Cptch_Package_List extends WP_List_Table {
		...
		function __construct() {
			parent::__construct( array(
				'singular'	=> __( 'package', 'captcha-plus' ),
				'plural'	=> __( 'packages', 'captcha-plus' ),
				'ajax'		=> false,
				)
			);
		}
		...
		function display_content() { ?>
			<h1 class="wp-heading-inline"><?php _e( 'Captcha Plus Packages', 'captcha-plus' ); ?></h1>
			<?php if ( $this->message ) { ?>
			...
			<form method="get" action="<?php echo self_admin_url( "admin.php" ); ?>" style="margin: 10px 0;">
				<?php $this->search_box( __( 'Search', 'captcha-plus' ), 'cptch_packages_search' );
		 ...
		 */
		function no_items() { ?>
			<p><?php _e( 'No packages found', 'captcha-plus' ); ?></p>
		<?php }
		...
		function get_columns() {
			return array(
				'name'	=> __( 'Package', 'captcha-plus' ),
				'date'	=> __( 'Date Added', 'captcha-plus' )
			);
		}
