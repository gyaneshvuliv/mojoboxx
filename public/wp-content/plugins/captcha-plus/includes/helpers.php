...
if ( ! function_exists( 'cptch_get_default_options' ) ) {
	function cptch_get_default_options() {
		...
		$default_options = array(
			...
			'time_limit'					=> 120,
			'no_answer'						=> __( 'Please complete the captcha.', 'captcha-plus' ),
			'wrong_answer'					=> __( 'Please enter correct captcha value.', 'captcha-plus' ),
			'time_limit_off'				=> __( 'Time limit exceeded. Please complete the captcha once again.', 'captcha-plus' ),
			'time_limit_off_notice'			=> __( 'Time limit exceeded. Please complete the captcha once again.', 'captcha-plus' ),
			'whitelist_message'				=> __( 'Your IP address is Whitelisted.', 'captcha-plus' ),
			'load_via_ajax'					=> false,
...
if ( ! function_exists( 'cptch_get_default_forms' ) ) {
	function cptch_get_default_forms() {
		$defaults = array(
			...
			'bws_contact', 'bws_booking', 'cf7_contact'
		);
...
if ( ! function_exists( 'cptch_parse_options' ) ) {
	function cptch_parse_options( $old_options, $default_options ) {
		...
		$args =array(
			...
			'bws_booking'		=> array(
				'enable'		=> array( 'cptchpls_booking', 'cptch_booking' )
			),
			'cf7_contact'		=> array(
				'enable'		=> 'cptchpls_cf7',
			)
		...
		$new_options['plugin_option_version']	= 'plus-' . $cptch_plugin_info["Version"];
...
if ( ! function_exists( 'cptch_get_plugin_link' ) ) {
	function cptch_get_plugin_link( $plugin ) {
		...
		switch ( $plugin ) {
			case 'contact-form-plugin/contact_form.php':
			case 'contact-form-pro/contact_form_pro.php':
				return sprintf( $bws_link, 'contact-form', '4aced5676cf54265add23f8f79e6b057' );
			...
			case 'limit-attempts-pro/limit-attempts-pro.php':
				return sprintf( $bws_link, 'limit-attempts', '7f52b8ba74b49875aeb687556c04efd8' );
			case 'contact-form-7/wp-contact-form-7.php':
